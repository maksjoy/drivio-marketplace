-- Transaction-only fixtures; always ROLLBACK. Run after supabase/migrations/20260907001412_production_readiness.sql.
begin;
create temporary table test_results(label text, passed boolean);
grant all on test_results to authenticated,anon;
create function pg_temp.assert_true(ok boolean,label text) returns void language plpgsql as $$begin if ok is distinct from true then raise exception 'FAILED: %',label;end if;insert into test_results values(label,true);end;$$;
create function pg_temp.expect_error(statement text,pattern text,label text) returns void language plpgsql as $$declare caught boolean:=false;begin begin execute statement;exception when others then if sqlerrm !~* pattern then raise exception 'Unexpected error in %: %',label,sqlerrm;end if;caught:=true;end;if not caught then raise exception 'FAILED (accepted): %',label;end if;insert into test_results values(label,true);end;$$;
...
insert into public.listings(id,user_id,seller_name,seller_email,make,model,year,price,mileage,fuel,status) values('20000000-0000-4000-8000-000000000001',auth.uid(),'Test','seller-test@example.invalid','Porsche','911',2020,50000,50000,'Gasoline','pending');
...
select pg_temp.expect_error($q$update public.listings set status='active' where id='20000000-0000-4000-8000-000000000001'$q$,'only mark','Owner cannot activate own listing');
...
insert into public.listings(user_id,seller_name,seller_email,make,model,year,price,mileage,fuel) select auth.uid(),'Test','test@example.invalid','Ford','Focus',2020,5000,50000,'Gasoline' from generate_series(1,2);
...
reset role;
-- Metadata only, within this rolled-back transaction; no actual storage upload is claimed here.
insert into storage.objects(bucket_id,name) values('listing-photos','10000000-0000-4000-8000-000000000001/20000000-0000-4000-8000-000000000001/test.jpg');
set local role authenticated;
insert into public.listing_images(listing_id,storage_path,position) values('20000000-0000-4000-8000-000000000001','10000000-0000-4000-8000-000000000001/20000000-0000-4000-8000-000000000001/test.jpg',0);
...
with updated as (update public.listings set status='removed' where id='20000000-0000-4000-8000-000000000001' returning id) select pg_temp.assert_true((select count(*) from updated)=0,'Another user cannot change listing');
...
select pg_temp.expect_error($q$insert into public.listing_reports(listing_id,reporter_id,reason) values('20000000-0000-4000-8000-000000000001',auth.uid(),'Duplicate')$q$,'duplicate key','Duplicate report rejected');
select set_config('request.jwt.claims','{"sub":"10000000-0000-4000-8000-000000000003","role":"authenticated"}',true);
