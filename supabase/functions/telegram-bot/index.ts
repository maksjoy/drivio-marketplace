...
export function createBotHandler(env:(name:string)=>string|undefined) {
 return async(req:Request)=>{
  ...
  const expected=(env('P2PCARS_TELEGRAM_WEBHOOK_SECRET')||'').trim();
  if(!expected)return new Response('Bot setup required',{status:503,headers:{'Cache-Control':'no-store'}});
  ...
  if(diff)return new Response('Unauthorized',{status:401,headers:{'Cache-Control':'no-store'}});
  try{
